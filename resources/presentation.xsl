...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:import href="/resources/components/slides.xsl" />
    <xsl:import href="/resources/components/layout-components.xsl" />
    <xsl:import href="/resources/components/content-components.xsl" />
    <xsl:import href="/resources/components/helpers.xsl" />

    <!-- Debug imports -->
    <xsl:template match="/">
        <xsl:variable name="test-import1" select="document('components/slides.xsl')"/>
        <xsl:variable name="test-import2" select="document('/resources/components/slides.xsl')"/>

        <xsl:comment>
            Import test relative path: <xsl:value-of select="boolean($test-import1)"/>
            Import test absolute path: <xsl:value-of select="boolean($test-import2)"/>
        </xsl:comment>
